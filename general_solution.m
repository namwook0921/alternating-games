...
for i = T: -1: 2
   ...
   % Compute Y2, Z2
   Y2_array(:, :, i) = S2_array(:, :, i) * B1 * S1_array(:, :, i) + T2_array(:, :, i) * S1_array ...
       (:, :, i) - S2_array(:, :, i) * A;
   Z2_array(:, :, i) = S2_array(:, :, i) * B1 * T1_array(:, :, i) + T2_array(:, :, i) * T1_array(:, :, i) - S2_array(:, :, i) * B2;
   
   % Compute Q2, R2, P2
   Q2_array(:, :, i - 1) = Q2 + V2_array(:, :, i)' * Q2_array(:, :, i) * V2_array(:, :, i) + ...
       Y2_array(:, :, i)' * R2_array(:, :, i) * Y2_array(:, :, i) + V2_array(:, :, i)' * P2_array(:, :, i) * Y2_array(:, :, i);
   R2_array(:, :, i - 1) = R2 + W2_array(:, :, i)' * Q2_array(:, :, i) * W2_array(:, :, i) + ...
       Z2_array(:, :, i - 1)' * R2_array(:, :, i) * Z2_array(:, :, i) + W2_array(:, :, i)' * P2_array(:, :, i) * Z2_array(:, :, i);
   ...
   % Compute Y1, Z1
   Y1_array(:, :, i) = S1_array(:, :, i) * B2 * S2_array(:, :, i - 1) + T1_array(:, :, i) * ...
       S2_array(:, :, i - 1) - S1_array(:, :, i) * A;
